        ...
        <tbody>
          {
            this.state.devices.map((dev, i) => (
              <tr key={dev.deviceId}>
                ...
                <td><CountUp key={dev.deviceId} end={(dev.DEVICE_ENERGY_CONSUMPTION)}     start={this.state.prev_devices[i].DEVICE_ENERGY_CONSUMPTION}     decimals={2}  suffix=" kWh (<i>monthly</i>)" /></td>
              </tr>
            ))
          }
        </tbody>
