...
def test_delete_v1_appmgr_apps_local_application_id_version_without_token(testapp):
    # type: (TestApp) -> None
    response = testapp.delete('/api/v1/appmgr/apps/local_app:1', expect_errors=True)
    assert response.status_code == 400


def test_delete_v1_appmgr_apps_local_application_id_version_no_application(testapp):
    # type: (TestApp) -> None
...
def test_delete_v1_appmgr_apps_local_application_id_version_with_application(testapp, database_logic, application):
